...
async def start():
    ...
    provider = AsyncOpenAI(
        api_key=secrets.gemini_api_key, base_url=secrets.gemini_base_url
    )
    ...
    agent = Agent(
        ...
        model=OpenAIChatCompletionsModel(
            model=secrets.gemini_api_model, openai_client=provider
        ),
    )
...
async def main(msg: cl.Message):
    ...
    msg = cl.Message(content=result.final_output)
    await msg.send()
